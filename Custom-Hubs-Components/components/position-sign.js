// JDB - practice creating a custom feature: this is a sign that show the latitude and longitude of the current position - which will be where the server is putatively located
function inject_create-position-sign() {
	...
	// NOT Interactable: set the class to interactable if you want interaction or some other class
	...
	// create a box geometry with a depth of .5 meters
	bh.value = "primitive: box; depth: 0.5";
	newEntity.setAttributeNode(bh);
				
	// reuse the same bh variable for a material attribute to color the geometry
	bh = document.createAttribute("material");
	// set the color to blue.  You can set a lot of things here, texture, shininess etc.  See the aframe docs on materials
	bh.value = "color:blue;metalness:1.0;roughness:0.0;";
	newEntity.setAttributeNode(bh);
	...
	// NOT NETWORKED Add the new schema to NAF. and declare the networked components and their update 
